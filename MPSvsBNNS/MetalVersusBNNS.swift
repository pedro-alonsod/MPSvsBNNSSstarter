...
class MetalVersusBNNS {
  ...
  private func initMPS() {
    ...
    // used to store the results of the BNNS matrix multiply).
  ...
  private func predictBNNS(logger: Logger) {
    let elapsed = timeIt {
    ...
    logger.log(message: "BNNS took \(elapsed) seconds")
  }
  ...
  private func compareResults(logger: Logger) {
    ...
    // Compare the output from MPS with the output from BNNS. Notice that there
